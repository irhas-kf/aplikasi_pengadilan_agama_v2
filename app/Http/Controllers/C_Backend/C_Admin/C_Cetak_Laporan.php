...
class C_Cetak_Laporan extends Controller
{

  public function index()
  {
    return view('V_Backend.V_Admin.V_Cetak_Laporan');
  }
  ...
  public function tampil_aksi(Request $request)
  {
    switch ($request->input('action')) {
        ...
        $th_saatini = date('Y');
           ...
           foreach ($tb_saldo as $tbsaldo) {
            ...
            $tbl_bulan2 = DB::select(
              "SELECT SUBSTRING(tb_perencanaan_anggaran.`tanggal_perencanaan_anggaran`, 6,2) AS bulan_pecah,
              tb_perencanaan_anggaran.*
              FROM tb_perencanaan_anggaran
              WHERE SUBSTRING(tb_perencanaan_anggaran.tanggal_perencanaan_anggaran, 1,4) = $filter_data_tahun_admin_session
              AND SUBSTRING(tb_perencanaan_anggaran.tanggal_perencanaan_anggaran, 6,2)
            ...
            ");
               ...
               $tbl_bulan3 = DB::select(
                 "SELECT SUBSTRING(tb_realisasi_anggaran.`tanggal_realisasi_anggaran`, 6,2) AS bulan_pecah1,
                 tb_realisasi_anggaran.*
                 FROM tb_realisasi_anggaran
                 WHERE SUBSTRING(tb_realisasi_anggaran.tanggal_realisasi_anggaran, 1,4) = $filter_data_tahun_admin_session
                 AND SUBSTRING(tb_realisasi_anggaran.tanggal_realisasi_anggaran, 6,2)
                 BETWEEN $bulanawal AND $bulanakhir
               ");
        ...
        return view('V_Backend.V_Admin.V_Cetak_View_Laporan',compact('nilai_bulan2','nominal_master','nilai_bulan3','nilai_realisasi_anggaran_persenan','nilai_perencanaan_anggaran_persenan','filter_data_tahun_admin_session'));
        ...
        $th_saatini = date('Y');
        $filter_data_tahun_admin_session = $request->session()->get('filter_data_tahun_admin');
        $perencanaamari = "DIPA BADILAG";
        $realisasimari = "DIPA BADILAG";
